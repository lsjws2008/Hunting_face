...
from archs import O_Net, losses
import time
...
parser.add_argument("-G", "--gpu-memory", help="gpu memary used", type=float, dest="gpu_memory", default="0.2")
...
with tf.Session(config=config) as sess:
    ...
    loss = 1 * tf.reduce_sum(cls_loss) + 0.5 * tf.reduce_sum(bbox_loss) + 1 * tf.reduce_sum(eye_reg_loss)
